...
Simplex_A::Simplex_A(double c[col - 1], double A[row - 1][col - 1], double b[row - 1], std::string var, std::string func)
{
	int i = 0, j = 0;
	for (i = 0; i < row - 1; i++)
	{
...
int Simplex_A::start()
{
	...
	else
	{
		...
		std::cout << "\nЦелевая функция " << leftstring[row - 1] << " = " << abs(Matrix[row - 1][0]) << std::endl;
	}
	return 0;
}


int Simplex_A::find_reference_solution()
{
	int i = 0, j = 0;
	int r = 0, k = 0;
...
int Simplex_A::search_good_column_to_swap(int r)
{
	int j = 0;
	for (j = 1; j < col; j++)
	{
...
int Simplex_A::find_optimal_solution()
{
	int i = row - 1, j = 0;
	int r = 0, k = 0;
	int count_for_while = 1;
...
int Simplex_A::search_good_row_to_swap(int k)
{
	int i = 0, j = 0;
	int imin = 0;
...
int Simplex_A::swap(int r, int k)
{
